<?xml version="1.0" encoding="utf-8"?>
<Root>
  <Header Id="7b347a37-6dce-4643-a9fb-c26fd678d8f7" ObjType="Nozzle" ObjName="HTE Nozzle (200µm)" Version="V2.0.17" />
  <Picture File="nozzle_ht_extruder.png">
    <Dimension X="5500" Y="4100" />
    <Offset X="-2750" Y="0" />
  </Picture>
  <PossibleParentTypes OnlySlots="TRUE">
    <Item>Extruder</Item>
  </PossibleParentTypes>
  <DefaultProperties>
    <Width Min="5500" Max="5500" Default="5500" Step="100" />
    <Depth Min="5500" Max="5500" Default="5500" Step="100" />
    <Height Min="4500" Max="4500" Default="4500" Step="100" />
    <OverlapTool Min="0" Max="0" Default="0" Step="10" />
    <InsideDiameter Min="100" Max="500" Default="200" Step="10" />
    <MeasureDistance Min="3000" Max="4500" Default="3500" Step="100" />
    <CleaningDepth Min="0" Max="15000" Default="2000" Step="100" />
  </DefaultProperties>
  <Categories>
    <Item>hte</Item>
  </Categories>
  <CleaningBrushCategories>
    <Item>stainlesssteel</Item>
    <Item>stainlesssteel+horizontal</Item>
    <Item>stainlesssteel+vertical</Item>
  </CleaningBrushCategories>
</Root>